...
| Key | Content |
|-----|---------|
| `scrum-facilitator-session` | `{ ceremonyType, stepIndex, totalSteps, currentStepId, completedSteps, participants, participantCount, retroNotes, retroNotesCount, retroFormat, teamName?, sprintGoal?, impediments?, savedAt }` — auto-saved during ceremony; `participantCount`, `retroNotesCount`, `totalSteps`, `currentStepId` are pre-computed for easy dashboard reads; `impediments` only populated for `daily` ceremonies |
